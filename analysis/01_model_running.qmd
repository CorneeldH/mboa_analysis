...
if(nrow(performance_summary) > 0) {
  ...
  cat("\nPerformance Comparison by Week Strategy:\n")
  print(knitr::kable(week_strategy_summary, digits = 4))
  
  # Visualize strategy comparison
  ggplot(performance_summary |> filter(.metric == "accuracy"), 
         aes(x = week_strategy, y = .estimate, fill = week_strategy)) +
    geom_boxplot() +
    labs(
      title = "Model Accuracy by Week Variable Strategy",
      x = "Week Strategy",
      y = "Accuracy"
    ) +
  ...
  if(nrow(program_levels_with_all_strategies) > 0) {
    ...
    direct_comparison <- performance_summary |>
      ...
      mutate(
        early_vs_none = none - early,
        all_vs_none = none - all,
        all_vs_early = early - all
      ) |>
      arrange(desc(all))
    
    cat("\nDirect comparison of week strategies (first 10 program-levels):\n")
    print(knitr::kable(head(direct_comparison, 10), digits = 4))
