...
students_satisfaction_enriched <- students_satisfaction |>
    left_join_load("crebo_to_organisation_mapping",
                   by = join_by(CREBO == OPLEIDING_crebo_correctie),
                   relationship = "many-to-one"
                   )
...
De studenttevredenheid uit de JOB light is op opleidingsniveau

```{r}

students_satisfaction_2020_file_path <- file.path(config::get("data_prepared_dir"), "students_satisfaction_2020.rds")

students_satisfaction_2022_file_path <- file.path(config::get("data_prepared_dir"), "students_satisfaction_2022.rds")

students_satisfaction_2020 <- readRDS(students_satisfaction_2020_file_path)
students_satisfaction_2022 <- readRDS(students_satisfaction_2022_file_path)
...
teams_students_satisfaction <- students_satisfaction_enriched |> 
    filter(niveau != "entreeopleiding") |>
    mutate(weegfac1 = as.numeric(weegfac1)) |>
    group_by(OPLEIDING_team, SCHOOLJAAR_startjaar) |>
    summarise(
...
teams_combined <- teams_enrollments |>
    ...
    left_join_load("teams_students_satisfaction",
              by = join_by(TEAM_naam == OPLEIDING_team, 
                           COHORT_startjaar == SCHOOLJAAR_startjaar),
              relationship = "one-to-one",
              .config_data_path = "data_combined_dir") |>
    left_join_load("teams_employee_satisfaction",
              by = join_by(TEAM_naam == characteristic_2, 
                           COHORT_startjaar == schooljaar_startjaar),
              relationship = "one-to-one",
              .config_data_path = "data_combined_dir")
