...
team_results_retention_start <- ingest_team_results_retention_start()
...
teams_results_retention_start <- teams_results_retention_start |>
    mutate(SCHOOLJAAR_naam = str_replace(SCHOOLJAAR_naam, " - ", "/"),
           # TODO parse numbers
           #EAM_startersresultaat_1_jaars = TEAM_startersresultaat_1_jaars * 100
           )
...
teams_correlation <- teams_combined %>%
    ...
    correlate() %>%
    rearrange() %>%
    select(term, TEAM_startersresultaat_1_jaars, everything()) %>%
    arrange(term != "TEAM_startersresultaat_1_jaars") %>%
    shave()

teams_correlation %>% rplot()

# test2 %>%
#     select(VERBINTENIS_volgnummer, BPV_omvang, COHORT_startjaar, VERBINTENIS_aanmelding_dagen_in_proces) %>%
#     correlate() %>%
#     rearrange() %>%
#     shave()
...
teams_correlation <- teams_combined %>%
  select(where(is.numeric)) %>%
  correlate() %>%
  # Focus only on TEAM_startersresultaat_1_jaars correlations
  select(term, TEAM_startersresultaat_1_jaars) %>%
  # Remove the self-correlation row
...
# For plotting
teams_correlation %>%
  rplot() #+  # Use sort=FALSE to maintain our manual sorting
  #coord_flip() +  # Flip coordinates to show variables on y-axis
  #theme(axis.text.y = element_text(size = 8))  # Adjust text size if needed

``
```{r}

ggplot(teams_correlation, 
       aes(x = "Startersresultaat 1 jaar", 
           y = term, 
           color = TEAM_startersresultaat_1_jaars)) +
  geom_point(size = 10) +  # Bigger dots
  geom_text(aes(label = sprintf("%.2f", TEAM_startersresultaat_1_jaars)), 
            hjust = -0.5, 
            size = 3) +
...
teams_correlation_plot <- teams_correlation %>%
  filter(TEAM_startersresultaat_1_jaars <= -0.2 | TEAM_startersresultaat_1_jaars >= 0.2) %>%
  arrange(TEAM_startersresultaat_1_jaars) %>% # Changed to descending order (0.50 at top)
  # Make term a factor with levels in the order we want
  mutate(term = factor(term, levels = term))

ggplot(teams_correlation_plot, 
       aes(x = "Startersresultaat 1 jaar", 
           y = term, 
           color = TEAM_startersresultaat_1_jaars)) +
  geom_point(size = 10) +
  geom_text(aes(label = sprintf("%.2f", TEAM_startersresultaat_1_jaars)), 
            hjust = -0.5, 
            nudge_x = 0.1,    # Moved text further right
            size = 3,
            color = "black") +
  scale_color_gradient2(
    low = "#ef6c6d",     # Lighter red (Light Salmon)
    mid = "white",
  ...
  theme(
    ...
    legend.position = "none"  # Removed the legend
  )
```
